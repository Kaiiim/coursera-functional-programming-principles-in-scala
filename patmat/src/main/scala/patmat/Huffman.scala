...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
        def pack(lst: List[Char]): List[List[Char]] = lst match {
          case Nil => Nil
          case head :: tail =>
            val (first, rest) = lst.span(y => y == head)
            first :: pack(rest)
        }
        val x = pack(chars.sorted)
        val y = x.map(ys => (ys.head, ys.length))
        y
      }
  /**
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
      ...
      def order(freqs: List[(Char, Int)]): List[Leaf] = freqs match {
        case Nil => Nil
        case head :: tail => Leaf(head._1, head._2) :: order(tail)
      }
      val x = order(freqs)
      val y = x.sortBy(weight)
      y
    }
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = {
      ...
      def putin(newtree: CodeTree, end: List[CodeTree]): List[CodeTree] = {
        if (end.isEmpty || weight(newtree) <= weight(end.head)) newtree :: end
        else end.head :: putin(newtree, end.tail)
      }

      trees match {
        case Nil => Nil
        case head :: Nil => head :: Nil
        case head :: between :: end => {
          val newtree = makeCodeTree(head, between)
          putin(newtree, end)
        }
      }
    }
  ...
  def createCodeTree(chars: List[Char]): CodeTree = {
    ...
    val llist = makeOrderedLeafList(list)
    until(singleton, combine)(llist).head
  }
   ...
   */
    def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
      def decodeAcc(innerTree: CodeTree, bits: List[Bit], acc: List[Char]): List[Char] = innerTree match {
